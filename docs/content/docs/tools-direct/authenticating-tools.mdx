...
#### Passing query parameters in the callback URL
...
<Callout type="info">
Your callback URL can include query parameters to carry context through the auth flow. Note that `initiate()` uses camelCase parameters (`connectedAccountId`, `appName`) in the redirect, unlike `link()` which uses snake_case (`connected_account_id`). See [Passing query parameters in the callback URL](#passing-query-parameters-in-the-callback-url) for details.
</Callout>
