...
If the `composio_managed_auth_schemes` array is empty, you'll need to provide your own credentials. You can also check on each toolkit's page on the [toolkits page](/toolkits). See [Using custom auth configuration](/docs/using-custom-auth-configuration) for a step-by-step walkthrough.
