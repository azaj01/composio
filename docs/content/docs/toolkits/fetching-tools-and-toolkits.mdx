...
You can filter to only connected toolkits or search by name:

<Tabs groupId="language" items={['Python', 'TypeScript']} persist>
<Tab value="Python">
```python
result = session.toolkits(is_connected=True)  # Only connected
result = session.toolkits(search="github")    # Search by name
```
</Tab>
<Tab value="TypeScript">
```typescript
...
const searched = await session.toolkits({ search: "github" });    // Search by name
```
</Tab>
</Tabs>
...
### Browse toolkits and tools

You can explore all 800+ toolkits at [platform.composio.dev](https://platform.composio.dev) or in the [docs](/toolkits).
