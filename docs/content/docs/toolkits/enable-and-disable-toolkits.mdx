...
You can also use the explicit `enable` syntax:
...
| `destructiveHint` | Tools that delete or permanently modify data |
| `idempotentHint` | Tools that can be safely retried |
| `openWorldHint` | Tools that interact with external systems |
...
### Read-only session for browsing

Create a safe session that can only read data:
...
### Restricted toolkits with full tool access

Limit to specific services but allow all their tools:
...
### Granular control per toolkit

Different restrictions for different services:
...
session = composio.create(
    ...
    tools={
        # Full access to GitHub
        # Gmail: only send and fetch
        "gmail": ["GMAIL_SEND_EMAIL", "GMAIL_FETCH_EMAILS"],
        # Slack: everything except delete
        "slack": {"disable": ["SLACK_DELETE_MESSAGE"]}
    }
)
```
