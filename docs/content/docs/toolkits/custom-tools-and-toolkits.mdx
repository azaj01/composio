...
const profiles: Record<string, { name: string; email: string; tier: string }> = {
  "user_1": { name: "Alice Johnson", email: "alice@acme.com", tier: "enterprise" },
  "user_2": { name: "Bob Smith", email: "bob@acme.com", tier: "free" },
};
...
// ── Extension tool ──────────────────────────────────────────────
// Wraps Gmail API with business logic. Inherits Gmail auth from the session
// via extendsToolkit, so ctx.proxyExecute() handles credentials automatically.
// This constrains the agent to send only the approved promo template.
...
const sendPromoEmail = experimental_createTool("SEND_PROMO_EMAIL", {
  ...
  execute: async (input, ctx) => {
    const subject = "You're invited to try Composio Pro";
    const body = "Hi there,\n\nWe'd love for you to try Composio Pro — free for 14 days.\n\nBest,\nThe Composio Team";
...
// ── Custom toolkit ──────────────────────────────────────────────
// Groups related tools under a namespace.
// Tools inside a toolkit must NOT use extendsToolkit — they inherit
// the toolkit identity instead.
...
- Slugs are auto-prefixed with `LOCAL_` (e.g. `GET_USER_PROFILE` → `LOCAL_GET_USER_PROFILE`)
- Toolkit tools get both prefixes (e.g. `LOCAL_USER_MANAGEMENT_ASSIGN_ROLE`)
