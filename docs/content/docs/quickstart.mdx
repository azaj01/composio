...
```bash
# Python
pip install composio composio-openai-agents openai-agents python-dotenv
...
result = Runner.run_sync(starting_agent=agent, input="Star the composiohq/composio repo on GitHub")
...
const result = await run(agent, "Star the composiohq/composio repo on GitHub");
...
Latest docs: https://docs.composio.dev/docs/quickstart
</PromptBanner>
...
```bash
# Python
pip install composio openai-agents python-dotenv

# TypeScript
npm install @composio/core @openai/agents zod@3 dotenv
```
...
- Set `require_approval` to `"never"` in the Python tool_config
...
4. Is `require_approval` set to `"never"` in Python?
...
# TypeScript
npm install @composio/core @composio/claude-agent-sdk @anthropic-ai/claude-agent-sdk dotenv
```
...
```bash
# Python
pip install composio claude-agent-sdk python-dotenv

# TypeScript
npm install @composio/core @anthropic-ai/claude-agent-sdk dotenv
```
...
const result = await generateText({
  ...
  tools,
  system: "You are a helpful assistant. Use Composio tools to take action.",
  prompt: "Star the composiohq/composio repo on GitHub",
  maxSteps: 10,
});
...
```bash
npm install @composio/core ai @ai-sdk/anthropic @ai-sdk/mcp dotenv
```
