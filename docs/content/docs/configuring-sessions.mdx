...
Allow users to connect multiple accounts for the same toolkit (e.g. personal and work Gmail). When enabled, the session can manage up to `maxAccountsPerToolkit` connections per toolkit.
...
|--------|------|---------|-------------|
| `enable` | `boolean` | org/project default | Enable multi-account mode for this session |
| `maxAccountsPerToolkit` | `number` (2-10) | 5 | Maximum connected accounts per toolkit |
| `requireExplicitSelection` | `boolean` | `false` | Require explicit account selection when multiple accounts are connected |

### Account aliases

Set human-readable aliases on connected accounts so users and agents can distinguish between them:
...
Aliases must be unique per entity and toolkit within a project.

You can also set an alias at creation time when initiating a new connection:

<Tabs groupId="language" items={['Python', 'TypeScript']} persist>
<Tab value="Python">
```python
# Set alias when creating a connected account
