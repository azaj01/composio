...
| `composio.trigger.disabled` | Fired when Composio automatically disables a trigger (expired connection, webhook refresh failure, or unhealthy polling). Not fired when you disable a trigger or its connected account yourself. |
