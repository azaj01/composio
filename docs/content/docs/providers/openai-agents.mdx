...
<Callout type="info">
Only use this if you want to use Composio's MCP server directly without the session abstraction.
</Callout>
...
# Get MCP URL directly
mcp = composio.mcp.get(user_id=external_user_id)
...
// Get MCP URL directly
const mcp = await composio.mcp.get(externalUserId);
