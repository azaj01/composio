---
title: Migration
description: "Endpoints to help with migration from v1 to v3"
---

{/* Auto-generated from OpenAPI spec. Do not edit directly. */}

Endpoints to help with migration from v1 to v3

## Endpoints

| Method | Path | Name | Description |
|--------|------|------|-------------|
| GET | `/api/v3/migration/get-nanoid` | [Get NanoId from UUID](/reference/api-reference/migration/get-nanoid-from-uuid) | Convert a legacy UUID to its corresponding NanoId for migration purposes. This endpoint facilitates the transition from UUID-based identifiers to the more compact NanoId format used in the v3 API. |
