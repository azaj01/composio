...
Or via the Webhook Subscriptions API — PATCH an existing subscription's `enabled_events`:
...
`user_id` is `null` on triggers from toolkits that do not require authentication. `trigger_config` holds the configuration you passed when creating the trigger — its fields vary per trigger type (the example above shows GitHub's `owner` / `repo`).
